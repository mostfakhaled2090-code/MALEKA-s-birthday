...
export default function LoadingScreen({ children }: LoadingScreenProps) {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-[#050101] text-white flex flex-col items-center justify-center font-sans selection:bg-white/10">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70vw] h-[70vw] rounded-full bg-red-900/15 blur-[120px] pointer-events-none z-0 mix-blend-screen" />
      ...
      >
        <div className="absolute top-1/4 left-1/4 w-[40vw] h-[40vw] rounded-full bg-rose-950/20 blur-[120px] mix-blend-screen" />
        <div className="absolute bottom-1/4 right-1/4 w-[50vw] h-[50vw] rounded-full bg-[#1a0005] blur-[150px] mix-blend-screen" />
      </div>
      ...
      <div className="relative z-30 flex flex-col items-center justify-center w-full max-w-4xl px-6 py-20 text-center">
        ...
        <div className="animate-stagger-4 mb-16 relative w-64 h-80 md:w-96 md:h-[32rem] rounded-[24px] overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(153,27,27,0.4)]">
          <Image 
            src="/m5.jpeg" 
...
function TimeUnit({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center w-16 md:w-24">
      {/* Hidden overflow to clip the sliding number */}
...
function RosePetals() {
  const petals = useMemo(() => {
    ...
    return Array.from({ length: 28 }).map((_, i) => ({
      ...
      blur: Math.random() > 0.5 ? `blur(${1 + Math.random() * 4}px)` : 'blur(0.5px)', 
  ...
  return (
    <div className="fixed inset-0 z-10 pointer-events-none overflow-hidden">
      {petals.map((p) => (
        ...
        >
          <svg viewBox="0 0 25 30" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-xl">
